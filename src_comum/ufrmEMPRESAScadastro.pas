...
uses
  ...
  ACBrEnterTab, JvExControls, JvSpeedButton, Vcl.ExtCtrls, Vcl.ComCtrls,
  JvExComCtrls, JvComCtrls, JvStatusBar, JvDBLookup, Vcl.StdCtrls, Vcl.DBCtrls,
  JvDBCheckBox, JvToolEdit, JvDBControls, Vcl.Mask, JvExMask, JvMaskEdit,
  JvExExtCtrls, JvShape, JvLabel, JvgGroupBox, FireDAC.Stan.Intf,
  FireDAC.Stan.Option, FireDAC.Stan.Param, FireDAC.Stan.Error, FireDAC.DatS,
  FireDAC.Phys.Intf, FireDAC.DApt.Intf, FireDAC.Stan.Async, FireDAC.DApt,
  FireDAC.Comp.DataSet, FireDAC.Comp.Client, JvExStdCtrls, JvCombobox, JvDBCombobox,
  JvSpin, JvDBSpinEdit, JvBaseEdits;
...
uses udtmDefault, ufrmMensagemEspera, ACBrUtil, ACBrValidador,
  uclassPESSOA;
...
begin
  ...
  if dtsDefault.DataSet.FieldByName('RAZAOSOCIAL').AsString.Trim.Length = 0 then
    ...
    dtmDefault.SetaGen('GEN_NUM_CTE_LJ'+(dtsDefault.DataSet.FieldByName('ID_EMPRESA').AsString),dtsDefault.DataSet.FieldByName('CTE_NUMERO_INICIAL').AsInteger);
    dtmDefault.SetaGen('GEN_NUM_MDFE_LJ'+(dtsDefault.DataSet.FieldByName('ID_EMPRESA').AsString),dtsDefault.DataSet.FieldByName('MDFE_NUMERO_INICIAL').AsInteger);
  inherited;
end;

procedure TfrmEMPRESAScadastro.FormClose(Sender: TObject;
...
procedure TfrmEMPRESAScadastro.FormCreate(Sender: TObject);
begin
  inherited;
  pageDefault.Pages[2].TabVisible:=False;  //CTe-Os
  pageDefault.Pages[4].TabVisible:=False;  //NFe
  pageDefault.Pages[5].TabVisible:=False;  //NFSe
end;
