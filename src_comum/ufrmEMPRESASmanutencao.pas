...
uses ufrmEMPRESAScadastro, uclassEMPRESA, ACBrUtil, udtmDefault;

procedure TfrmEMPRESASmanutencao.btnSelecionarEmitenteClick(Sender: TObject);
var
  AMsg: String;
begin
  AMsg := 'Deseja inicar o Emitente selecionado ?' + sLineBreak + sLineBreak + 'EMITENTE: ' + qryDefaultRAZAOSOCIAL.AsString;
  ...
  begin
    if not dtmDefault.ChecaExisteGen('GEN_NUM_CTE_LJ'+InttoStr(oEmpresa.ID)) then
     begin
       dtmDefault.CriarGen('GEN_NUM_CTE_LJ'+InttoStr(oEmpresa.ID));
     end;
     if not dtmDefault.ChecaExisteGen('GEN_NUM_MDFE_LJ'+InttoStr(oEmpresa.ID)) then
     begin
       dtmDefault.CriarGen('GEN_NUM_MDFE_LJ'+InttoStr(oEmpresa.ID));
     end;
...
procedure TfrmEMPRESASmanutencao.dtsDefaultDataChange(Sender: TObject;
  Field: TField);
begin
  inherited;

  btnSelecionarEmitente.Enabled
    := Assigned(dtsDefault.DataSet) and (TFDQuery(dtsDefault.DataSet).RecordCount > 0);
end;
